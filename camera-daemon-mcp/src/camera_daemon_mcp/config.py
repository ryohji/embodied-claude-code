"""Configuration for Camera Daemon MCP Server."""
...
class ServerConfig:
    ...
    name: str = "camera-daemon-mcp"
    version: str = "0.1.0"
    capture_dir: str = "/tmp/camera-daemon-mcp"

    @classmethod
    def from_env(cls) -> "ServerConfig":
        """Create config from environment variables."""
        return cls(
            name=os.getenv("MCP_SERVER_NAME", "camera-daemon-mcp"),
            version=os.getenv("MCP_SERVER_VERSION", "0.1.0"),
            capture_dir=os.getenv("CAPTURE_DIR", "/tmp/camera-daemon-mcp"),
        )
