...
Claude Code に「listen」と送ると、マイクで録音 → Whisper で書き起こし → テキストとして返ってくる。Claude Code はその内容に応じて `say` ツールで声を出して返答する。

```
あなた: listen
Claude: (マイクで5秒間録音)
Claude: 聞こえました。「こんにちは」とおっしゃっていますね。
Claude: (スピーカーから「こんにちは！何かお手伝いできることはありますか？」と発話)
```
